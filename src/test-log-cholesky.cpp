...
context("log-cholesky works as expected") {
  test_that("log_chol::pd_mat works as expected") {
    ...
    constexpr double theta[(dim * (dim + 1)) / 2] = { 0.253929615612238, 1.2724293214294, 0.856338430490728, -1.53995004190371, -0.928567034713538, 0.25711649595832, -0.00576717274753696, 2.40465338885795, 0.763593461140459, -0.441421449571498 };
    double res[dim * dim];
    ...
    std::unique_ptr<double[]>
      mem(new double[log_chol::pd_mat::get_n_dmen(dim)]);
