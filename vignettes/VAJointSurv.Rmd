...
The package currently requires the development version of the psqn package. 
Thus, you need to install it, e.g. by calling

```{r install_psqn_devel, eval = FALSE}
...
The package also supplies an approximation of the observed information matrix
...
off and either cause slow convergence or problems during the optimization. In 
such case, we can change the starting values with the `joint_ms_set_vcov` 
function as shown below.

```{r change_start_val, cache = 1}
