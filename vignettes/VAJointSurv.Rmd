...
section. The examples includes: 
 ...
 - An example with two markers, the observation process, and a terminal event 
   ...
   dependence accounted for in some other way.
...
[Survival Outcomes](#survival-outcomes) has to be done many times. Thus, we have
implemented all the expansions in C++ to be used in C++ to reduce the cost of 
the evaluations. 

The `_term` functions provide the R interface to the C++ functions. Each 
return a list with elements like their R counterparts and a `eval` element. 
The latter element can be used to evaluate the basis, the derivatives of the 
basis if the `der` argument is less then zero, or the integral 

$$\vec M(u) = \int_l^u \vec m(s) ds$$

if the `der` argument is equal to minus one. The lower limits is passed with 
...
# derivatives
t(b$eval(4.33, der = 1))
f <- function(z) predict(bs(xx, df = 4, Boundary.knots = range(xx)), z)
t(jacobian(f, 4.33))

# integrals
t(b$eval(4, der = -1, lower_limit = 0))
do_int(4, f, 0)
t(b$eval(4, der = -1, lower_limit = 1))
do_int(4, f, 1)
...
# integrals
t(n$eval(4.33, der = -1, lower_limit = 0))
do_int(4.33, f, 0)
```

Caution: integration is not implemented for the B-splines or natural cubic 
splines outside of the boundary knots!
...
A quadrature is used to integrate the expected cumulative hazard. The user can 
pass any 
