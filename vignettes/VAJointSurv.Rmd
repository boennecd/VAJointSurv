...
vignette: >
  %\VignetteIndexEntry{Joint Survival and Marker Models}
...
by covering the model, then cover some examples, and end with details 
...
The following is not supported
 ...
 - One may either want the integral or the derivatives of $M(t)$ with 
   respect to $t$ in the log hazards.
   This is not too complicated to implement for many of the 
   basis expansions in the package but it is not yet implemented.
 - One may want to evaluate $\vec b_i$ at $\log t$ rather than $t$ in the log
   hazard. This is not too complicated to implement but not yet implemented.
 - There cannot be multiple observed markers of the same type at each point
   $s_{ij}$ for each individual $i$.
 - There cannot be time-varying effects for a covariate. 
   This is not too complicated to implement but not yet implemented. 
 - Competing events, terminal events, and multi-state models are not supported.
...
Psi <- structure(c(0.18, 0.05, -0.05, 0.05, 0.34, -0.25, -0.05, -0.25, 0.24), 
                 .Dim = c(3L, 3L))
Sig <- matrix(.6^2, 1)
...
plot_marker(
  ...
  fixef_vary = fixef_vary_marker, x_range = c(0, 2), vcov_vary = Psi)
```

```{r sim_n_fit_univariate}
library(mvtnorm)
...
Psi <- structure(c(0.35, 0.02, -0.05, 0.01, 0.02, 0.12, -0.06, -0.01, -0.05, -0.06, 0.32, 0.09, 0.01, -0.01, 0.09, 0.12),
                 .Dim = c(4L, 4L))
Sig <- matrix(c(.6^2, .1, .1, .4^2), 2)
...
plot_marker(
  ...
  vcov_vary = Psi[1:2, 1:2], ylab = "Marker 1")
plot_marker(
  ...
  vcov_vary = Psi[3:4, 3:4], ylab = "Marker 2")
```

```{r sim_n_fit_two_markers}
library(mvtnorm)
# simulates from the model by sampling a given number of individuals
sim_dat <- function(n_ids){
  # simulate the outcomes
  dat <- lapply(1:n_ids, function(id){
    ...
    ys <- eta + rmvnorm(n_obs, sigma = Sig)
...
surv_vals <- replicate(100, {
  err <- rnorm(1, sd = sqrt(Xi))
  eval_surv_base_fun(
    ti = tis, omega = omega, b_func = b_func, 
    gl_dat = get_gl_rule(100), delta = delta[1] + err)
})
...
sim_dat <- function(n_ids){
  ...
  dat <- lapply(1:n_ids, function(id){
    ...
    rng_surv <- rnorm(1, sd = sqrt(Xi))
    Z <- c(1, runif(1, -1, 1))
    log_haz_offset <- sum(Z * delta) + rng_surv

    # the conditional survival function
    surv_func <- function(ti)
      eval_surv_base_fun(
        ti = ti, omega = omega, b_func = b_func, gl_dat = gl_dat,
...
rbind(estimate = fmt_est$survival[[1]]$fixef, truth = delta)
rbind(estimate = fmt_est$survival[[1]]$fixef_vary, truth = omega)
c(estimate = sqrt(fmt_est$vcov$vcov_surv), truth = sqrt(Xi))
```

#### Note on Quadrature Rule

A quadrature is used to integrate the cumulative hazard. The user can pass any 
...
to use a midpoint rule. This is illustrated below.

```{r trap_rule}
# computes a midpoint rule
mid_rule <- function(n)
  list(node = seq(0, 1, length.out = n), weight = rep(1 / n, n))

# compare with a midpoint with different number of nodes
for(n in 2^(3:12)){
...
Evaluating the cumulative hazard is by far the most computationally expensive 
...
plot_surv(
  time_fixef = bs_term(knots = 1, Boundary.knots = c(0, 2)), 
  time_rng = list(
    ns_term(knots = numeric(), Boundary.knots = c(0, 2), intercept = TRUE), 
    poly_term(degree = 1, raw = TRUE, intercept = TRUE)), 
  x_range = c(0, 2), vcov_vary = Psi, frailty_var = Xi,
  ps = c(.25, .5, .75), log_hazard_shift = delta[1], 
  fixef_vary = omega, associations = alpha)
...
plot_surv(
  ...
  time_rng = list(
    ns_term(knots = numeric(), Boundary.knots = c(0, 2), intercept = TRUE), 
    poly_term(degree = 1, raw = TRUE, intercept = TRUE)), 
  x_range = c(0, 2), vcov_vary = matrix(0, 4, 4), frailty_var = Xi,
  ps = c(.25, .5, .75), log_hazard_shift = delta[1], 
  fixef_vary = omega, associations = alpha)
...
tis <- seq(0, 2, length.out = 50)
set.seed(1)
Us <- mvtnorm::rmvnorm(250, sigma = Psi) # the random effects
# compute the conditional survival functions disregarding the fixed effect
surv_vals <- apply(Us, 1, function(U){
  ...
  eval_surv_base_fun(
    ti = tis, omega = c(omega, alpha), b_func = expansion, 
    gl_dat = get_gl_rule(100), delta = delta[1] + err)
})
...
sim_dat <- function(n_ids){
  ...
  dat <- lapply(1:n_ids, function(id){
    ...
    surv_func <- function(ti)
      eval_surv_base_fun(
        ti = ti, omega = c(omega, alpha), b_func = expansion,
...
all.equal(numDeriv::grad(f, head(start_val, 31 + 2 * 20)), 
          head(joint_ms_lb_gr(comp_obj, start_val), 31 + 2 * 20))
...
# the survival parameters
vcov_frailty <- matrix(c(.2^2, .15^2, .15^2, .25^2), 2)
...
plot_surv(
  ...
  vcov_vary = vcov_vary, frailty_var = vcov_frailty[1, 1], ps = c(.25, .5, .75),
...
plot_surv(
  ...
  vcov_vary = vcov_vary, frailty_var = vcov_frailty[2, 2], ps = c(.25, .5, .75),
  associations = associations[[2]], log_hazard_shift = fixef_surv[[2]][1],
  ylab = "Observation process")
```

```{r sim_fit_obs_process_markers_and_recurrent}
library(mvtnorm)
# simulates from the model by sampling a given number of individuals
sim_dat <- function(n_ids){
  # simulate the outcomes
  ...
  dat <- lapply(1:n_ids, function(id){
    ...
    frailties <- drop(rmvnorm(1, sigma = vcov_frailty))
    Z1 <- c(1, runif(1, -1, 1))
    ...
    expansion <- function(x, b_func)
      cbind(b_func(x), m_funcs[[1]](x) %*% U[1:2],
            m_funcs[[2]](x) %*% U[3:4])
    surv_func <- function(ti, omega, alpha, b_func){
    ...
    root_func <- function(x, rng)
      rng - surv_func(x, omega = fixef_vary_surv[[1]],
                      alpha = associations[[1]], b_func = b_funcs[[1]])
    ...
    root_func <- function(x, left_trunc_surv, rng)
      rng - surv_func(x, omega = fixef_vary_surv[[2]],
                      alpha = associations[[2]], b_func = b_funcs[[2]]) /
      left_trunc_surv
    ...
    for(i in 1:max_sample){
      ...
      left_trunc_surv <- surv_func(
        y[i], omega = fixef_vary_surv[[2]], alpha = associations[[2]],
        b_func = b_funcs[[2]])
    }
