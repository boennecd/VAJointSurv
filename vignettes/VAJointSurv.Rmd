...
The following is not supported
 ...
 - One may want either want the integral or the derivatives of $M(t)$ with 
 ...
 - Competing events, terminal events, multi-state models are not supported.
   
   
## Examples
We show a few examples of this package and its API in this section.
...
lme4 should used in this case as the linear model is tractable and this is used
...
library(splines)
g_func <- function(x)
  ns(x, knots = c(.5, 1.5), Boundary.knots = c(0, 2), intercept = FALSE)
m_func <- function(x)
...
plot_marker(
  time_fixef = ns_term(
    knots = c(.5, 1.5), Boundary.knots = c(0, 2), intercept = FALSE),
  time_rng = ns_term(
...
sim_dat_univariate <- function(n_ids){
...
solution in this case. Thus, this is not the optimal way of doing this.
...
b_func <- function(x)
  bs(x, knots = 1, Boundary.knots = c(0, 2), intercept = FALSE)

# draw a few survival curves an plot them 
...
# compute the average to get an estimate of the marginal hazard
...
surv_obj <- surv_term(
  ...
  time_fixef = bs_term(y, knots = c(.5, 1.5), Boundary.knots = c(0, 2)))
...
# the fixed effects for the survival outcome
fmt_ests$survival[[1]]$fixef
delta
fmt_ests$survival[[1]]$fixef_vary
omega
fmt_ests$survival[[1]]$associations
alpha
...
# the parameters for the error term covariance
fmt_ests$vcov$vcov_marker
Sig

# the parameters for the frailty covariance matri
